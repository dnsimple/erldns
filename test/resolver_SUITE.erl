...
%% Tests
resolve_no_question_returns_message_test(_) ->
    Q = #dns_message{questions = []},
    ?assertEqual(Q, erldns_resolver:resolve(Q, [], {1, 1, 1, 1})).

resolve_rrsig_refused_test(_) ->
...
resolve_no_authority_refused_test(_) ->
    Q = #dns_message{
...
resolve_authoritative_host_not_found_test(_) ->
...
resolve_authoritative_zone_cut_test(_) ->
    erldns_zone_cache:start_link(),
...
resolve_authoritative_zone_cut_with_cnames_test(_) ->
    erldns_zone_cache:start_link(),
    erldns_handler:start_link(),
    Qname = <<"delegated.example.com">>,
    CnameRecords =
