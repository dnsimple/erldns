...
throttle(Message, {_, Host}) ->
    case ?ENABLED of
        true ->
            case lists:filter(fun(Q) -> Q#dns_query.type =:= ?DNS_TYPE_ANY end,
                              Message#dns_message.questions) of
                [] -> ok;
...
handle_cast(sweep, State) ->
    Keys = erldns_storage:select(host_throttle,
                                 [{{'$1', {'_', '$2'}}, [{'<', '$2', timestamp() - ?EXPIRATION}], ['$1']}]),
