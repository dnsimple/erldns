...
resolve(Message, Qname, Qtype, _MatchedRecords = [], {ClientIP, ServerIP}, CnameChain, Zone) ->
  %erldns_log:debug("No exact matches, using best-match resolution"),
  best_match_resolution(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, best_match(Qname, Zone), Zone);

%% There was at least one exact match on name.
resolve(Message, Qname, Qtype, MatchedRecords, {ClientIP, ServerIP}, CnameChain, Zone) ->
  %erldns_log:debug("Exect match on name ~p (records: ~p)", [Qname, MatchedRecords]),
...
exact_match_resolution(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone) ->
  ...
  exact_match_resolution(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, CnameRecords).

%% No CNAME records found in the records with the Qname
exact_match_resolution(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, _CnameRecords = []) ->
...
resolve_exact_match(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone) ->
  ...
  case TypeMatches of
    [] ->
      ...
      %erldns_log:debug("Exact match for name, but no type matches in zone, try custom handlers"),
      NewRecords = lists:flatten(lists:map(custom_lookup(Qname, Qtype, MatchedRecords), erldns_handler:get_handlers())),
      resolve_exact_match(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, NewRecords, AuthorityRecords);
    _ ->
      %erldns_log:debug("Found exact match for name and type matches"),
      resolve_exact_match(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, TypeMatches, AuthorityRecords)
  end.

%% There were no matches for exact name and type, so now we are looking for NS records
%% in the exact name matches.
resolve_exact_match(Message, _Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, _ExactTypeMatches = [], AuthorityRecords) ->
  %erldns_log:debug("No exact matches for name and type"),
  ReferralRecords = lists:filter(erldns_records:match_type(?DNS_TYPE_NS), MatchedRecords), % Query matched records for NS type
  resolve_no_exact_type_match(Message, Qtype, {ClientIP, ServerIP}, CnameChain, [], Zone, MatchedRecords, ReferralRecords, AuthorityRecords);

%% There were exact matches of name and type.
resolve_exact_match(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, _MatchedRecords, Zone, ExactTypeMatches, AuthorityRecords) ->
  %erldns_log:debug("Found exact matches of name and type. Authority records: ~p", [AuthorityRecords]),
  resolve_exact_type_match(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, ExactTypeMatches, Zone, AuthorityRecords).



%% There was an exact type match for an NS query, however there is no SOA record for the zone.
resolve_exact_type_match(Message, _Qname, ?DNS_TYPE_NS, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, []) ->
  Answer = lists:last(MatchedRecords),
  Name = Answer#dns_rr.name,
  %erldns_log:debug("Exact match type was NS, restarting query with delegated name ~p", [Name]),
  % It isn't clear what the QTYPE should be on a delegated restart. I assume an A record.
  restart_delegated_query(Message, Name, ?DNS_TYPE_A, {ClientIP, ServerIP}, CnameChain, Zone, erldns_zone_cache:in_zone(Name));

%% There was an exact type match for an NS query and an SOA record.
resolve_exact_type_match(Message, _Qname, ?DNS_TYPE_NS, {_ClientIP, _ServerIP}, _CnameChain, MatchedRecords, _Zone, _AuthorityRecords) ->
  %erldns_log:debug("Authoritative for record, returning answers"),
...
resolve_exact_type_match(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, _AuthorityRecords) ->
  ...
  case erldns_zone_cache:get_delegations(Answer#dns_rr.name) of
    [] ->
      resolve_exact_type_match(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, _AuthorityRecords, _NSRecords = []);
    NSRecords  ->
        ...
        {ok, [SoaRecord]} ->
          case SoaRecord#dns_rr.name =:= NSRecord#dns_rr.name of
            true ->
              Message#dns_message{aa = true, rc = ?DNS_RCODE_NOERROR, answers = Message#dns_message.answers ++ MatchedRecords};
            false ->
              resolve_exact_type_match(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, _AuthorityRecords, NSRecords)
          end;
...
resolve_exact_type_match(Message, _Qname, _Qtype, {_ClientIP, _ServerIP}, _CnameChain, MatchedRecords, _Zone, _AuthorityRecords, _NSRecords = []) ->
  Message#dns_message{aa = true, rc = ?DNS_RCODE_NOERROR, answers = Message#dns_message.answers ++ MatchedRecords};

%% We are authoritative and there are NS records here.
resolve_exact_type_match(Message, _Qname, Qtype, {ClientIP, _ServerIP}, CnameChain, MatchedRecords, Zone, _AuthorityRecords, NSRecords) ->
  ...
  case Name =:= Answer#dns_rr.name of
    ...
    false ->
      ...
      case check_if_parent(Name, Answer#dns_rr.name) of
        true ->
          restart_delegated_query(Message, Name, Qtype, {ClientIP, _ServerIP}, CnameChain, Zone, erldns_zone_cache:in_zone(Name));
        false ->
          Message#dns_message{aa = true, rc = ?DNS_RCODE_NOERROR, answers = Message#dns_message.answers ++ MatchedRecords}
      end
  end.
...
resolve_no_exact_type_match(Message, ?DNS_TYPE_ANY, {_ClientIP, _ServerIP}, _CnameChain, _ExactTypeMatches, _Zone, [], [], AuthorityRecords) ->
  Message#dns_message{aa = true, authority = AuthorityRecords};
resolve_no_exact_type_match(Message, _Qtype, {_ClientIP, _ServerIP}, _CnameChain, [], Zone, _MatchedRecords, [], _AuthorityRecords) ->
  Message#dns_message{aa = true, authority = Zone#zone.authority};
resolve_no_exact_type_match(Message, _Qtype, {_ClientIP, _ServerIP}, _CnameChain, ExactTypeMatches, _Zone, _MatchedRecords, [], _AuthorityRecords) ->
  Message#dns_message{aa = true, answers = Message#dns_message.answers ++ ExactTypeMatches};
resolve_no_exact_type_match(Message, Qtype, {_ClientIP, _ServerIP}, _CnameChain, _ExactTypeMatches, _Zone, MatchedRecords, ReferralRecords, AuthorityRecords) ->
...
resolve_exact_match_referral(Message, ?DNS_TYPE_ANY, MatchedRecords, _ReferralRecords, _AuthorityRecords) ->
  %erldns_log:debug("Exact match ANY"),
...
% the answers section..
resolve_exact_match_with_cname(Message, ?DNS_TYPE_CNAME, {_ClientIP, _ServerIP}, _CnameChain, _MatchedRecords, _Zone, CnameRecords) ->
...
resolve_exact_match_with_cname(Message, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, CnameRecords) ->
  resolve_exact_match_with_cname(Message, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, CnameRecords, lists:member(lists:last(CnameRecords), CnameChain)).

%% Indicates a CNAME loop. The response code is a SERVFAIL in this case.
resolve_exact_match_with_cname(Message, _Qtype, {_ClientIP, _ServerIP}, _CnameChain, _MatchedRecords, _Zone, _CnameRecords, true) ->
  %erldns_log:debug("CNAME loop detected (exact match)"),
  Message#dns_message{aa = true, rc = ?DNS_RCODE_SERVFAIL};
% No CNAME loop, restart the query with the CNAME content.
resolve_exact_match_with_cname(Message, Qtype, {ClientIP, ServerIP}, CnameChain, _MatchedRecords, Zone, CnameRecords, false) ->
  CnameRecord = lists:last(CnameRecords),
  Name = CnameRecord#dns_rr.data#dns_rrdata_cname.dname,
  %erldns_log:debug("Restarting query with CNAME name ~p (exact match)", [Name]),
  restart_query(Message#dns_message{aa = true, answers = Message#dns_message.answers ++ CnameRecords}, Name, Qtype, {ClientIP, ServerIP}, CnameChain ++ CnameRecords, Zone, erldns_zone_cache:in_zone(Name)).
...
restart_delegated_query(Message, Name, Qtype, {ClientIP, ServerIP}, CnameChain, Zone, false) ->
  resolve(Message, Name, Qtype, erldns_zone_cache:find_zone(Name, Zone#zone.authority), {ClientIP, ServerIP}, CnameChain). % Zone lookup
...
best_match_resolution(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, BestMatchRecords, Zone) ->
  %erldns_log:debug("No exact match found, using ~p", [BestMatchRecords]),
...
resolve_best_match(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, BestMatchRecords, Zone) ->
  %erldns_log:debug("No referrals found"),
  resolve_best_match(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, BestMatchRecords, Zone, lists:any(erldns_records:match_wildcard(), BestMatchRecords)).

%% It's a wildcard match
resolve_best_match(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, BestMatchRecords, Zone, true) ->
  %erldns_log:debug("Matched records are wildcard."),
  ...
  resolve_best_match_with_wildcard(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, BestMatchRecords, Zone, CnameRecords);
%% It is not a wildcard.
resolve_best_match(Message, Qname, _Qtype, {_ClientIP, _ServerIP}, _CnameChain, _BestMatchRecords, Zone, false) ->
  %erldns_log:debug("Matched records are not wildcard."),
...
resolve_best_match_with_wildcard(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, []) ->
  %erldns_log:debug("Resolving best match with wildcard"),
  TypeMatchedRecords = case Qtype of
                         ?DNS_TYPE_ANY ->
                           FilteredMatchedRecords = filter_records(MatchedRecords, erldns_handler:get_handlers()),
                           %erldns_log:debug("Qtype is ANY, original records: ~p; using records ~p", [MatchedRecords, FilteredMatchedRecords]),
  ...
  case TypeMatches of
    [] ->
      %% Ask the custom handlers for their records.
      %erldns_log:debug("No type matches found, using custom handlers"),
      ...
      %erldns_log:debug("Records from custom handlers: ~p", [NewRecords]),
      resolve_best_match_with_wildcard(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, [], NewRecords);
    _ ->
      resolve_best_match_with_wildcard(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, MatchedRecords, Zone, [], TypeMatches)
  end;

% It is a wildcard CNAME
resolve_best_match_with_wildcard(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, BestMatchRecords, Zone, CnameRecords) ->
  resolve_best_match_with_wildcard_cname(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, BestMatchRecords, Zone, CnameRecords).

% It is not a CNAME and there were no exact type matches
resolve_best_match_with_wildcard(Message, _Qname, _Qtype, {_ClientIP, _ServerIP}, _CnameChain, _BestMatchRecords, Zone, [], []) ->
  Message#dns_message{aa = true, authority=Zone#zone.authority};

% It is not a CNAME and there were exact type matches
resolve_best_match_with_wildcard(Message, _Qname, _Qtype, {_ClientIP, _ServerIP}, _CnameChain, _BestMatchRecords, _Zone, [], TypeMatches) ->
...
resolve_best_match_with_wildcard_cname(Message, _Qname, ?DNS_TYPE_CNAME, {_ClientIP, _ServerIP}, _CnameChain, _BestMatchRecords, _Zone, CnameRecords) ->
  Message#dns_message{aa = true, answers = Message#dns_message.answers ++ CnameRecords};
...
resolve_best_match_with_wildcard_cname(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, BestMatchRecords, Zone, CnameRecords) ->
  ...
  resolve_best_match_with_wildcard_cname(Message, Qname, Qtype, {ClientIP, ServerIP}, CnameChain, BestMatchRecords, Zone, CnameRecords, lists:member(CnameRecord, CnameChain)).

% Indicates CNAME loop
resolve_best_match_with_wildcard_cname(Message, _Qname, _Qtype, {_ClientIP, _ServerIP}, _CnameChain, _BestMatchRecords, _Zone, _CnameRecords, true) ->
  %erldns_log:debug("CNAME loop detected (best match)"),
  Message#dns_message{aa = true, rc = ?DNS_RCODE_SERVFAIL};

% We should follow the CNAME
resolve_best_match_with_wildcard_cname(Message, _Qname, Qtype, {ClientIP, ServerIP}, CnameChain, _BestMatchRecords, Zone, CnameRecords, false) ->
  %erldns_log:debug("Follow CNAME (best match)"),
  CnameRecord = lists:last(CnameRecords),
  Name = CnameRecord#dns_rr.data#dns_rrdata_cname.dname,
  restart_query(Message#dns_message{aa = true, answers = Message#dns_message.answers ++ CnameRecords}, Name, Qtype, {ClientIP, ServerIP}, CnameChain ++ CnameRecords, Zone, erldns_zone_cache:in_zone(Name)).
...
resolve_best_match_referral(Message, _Qname, _Qtype, {_ClientIP, _ServerIP}, [], _BestMatchRecords, _Zone, _ReferralRecords, Authority) ->
...
% original message.
resolve_best_match_referral(Message, _Qname, ?DNS_TYPE_ANY, {_ClientIP, _ServerIP}, _CnameChain, _BestMatchRecords, _Zone, _ReferralRecords, _Authority) ->
  Message;
