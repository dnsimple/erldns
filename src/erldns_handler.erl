...
resolve(Message, AuthorityRecords, Host, Question) when is_record(Question, dns_query) ->
  ...
  lager:debug("Starting resolution for ~p", [Host]),
  ResolvedMessage = resolve(Message#dns_message{ra = false}, AuthorityRecords, Question#dns_query.name, Question#dns_query.type, Host),
  lager:debug("Completed resolution for ~p", [Host]),
  ResolvedMessage.
